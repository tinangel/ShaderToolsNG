...
def ApiKeys():
    temp = \
        (      
         ...
         "texture_slots_add", "texture_image_save_render", "texture_image_save_as",)
    return temp
#end Api keys
#Langages keys
def LangagesKeys():
    temp = \
        [      
         ...
         "menu_configuration_default_config", "bl_id_config_search", "menu_configuration_langage_choice", "menu_configuration_current_index", "take_preview",
         ]
    for i in range(1, 31):
...
def ImageFileFormatKeys(format):
    ...
    return format_supported[format]
